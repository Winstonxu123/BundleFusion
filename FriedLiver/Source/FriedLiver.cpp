...
//correspondence search & global pose estimation
...
void bundlingOptimizationThreadFunc() {
	...
	DualGPU::get().setDevice(DualGPU::DEVICE_BUNDLING);  //´ÎGPU£¬ÓÃÀ´ÊµÏÖ¶ÔÓ¦µãËÑË÷ºÍÈ«¾ÖÓÅ»¯
...
void bundlingThreadFunc() {
	...
	while (1) {
		...
		if (g_RGBDSensor->isReceivingFrames()) {
			if (g_bundler->getCurrProcessedFrame() % 10 == 0) { // stop solve，前9帧需要等
				if (tOpt.joinable()) {
					tOpt.join();
				}
			}
			if (g_bundler->getCurrProcessedFrame() % 10 == 1) { // start solve
				...
				tOpt = std::thread(bundlingOptimizationThreadFunc);//Ã¿11Ö¡×é³ÉÒ»¸öchunk½øÐÐ¿éÄÚÓÅ»¯local
			}
		}
			...
			tOpt = std::thread(bundlingOptimizationThreadFunc);//globalÓÅ»¯
		}
...
int main(int argc, char** argv)
{
	...
	try {
		...
		if (argc == 4) //for scan net: overwrite .sens file
		{
			...
			std::cout.rdbuf(out.rdbuf()); //redirect std::cout to out.txt!Êä³öÖØ¶¨Ïòµ½ÎÄ¼þ
		...
		dualGPU.setDevice(DualGPU::DEVICE_RECONSTRUCTION);	//main gpu£¬µÚÒ»¿éGPUÓÃÓÚÌåÖØ½¨£¬µÚ¶þ¿éGPUÓÃÓÚ¶ÔÓ¦µãËÑË÷ÓëÈ«¾ÖÓÅ»¯
		...
		g_RGBDSensor->createFirstConnected(); //在这里选用直接读取，如果以后要修改这里需要变


		g_imageManager = new CUDAImageManager(GlobalAppState::get().s_integrationWidth, GlobalAppState::get().s_integrationHeight,
			GlobalBundlingState::get().s_widthSIFT, GlobalBundlingState::get().s_heightSIFT, g_RGBDSensor, false); //false代表 storeFramesOnGPU
